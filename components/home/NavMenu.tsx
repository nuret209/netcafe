        ...
        <nav className='border fixed z-40 border-[#ebebeb] bg-white md:w-[88px] left-0 md:top-2 bottom-0 md:left-2 md:h-[calc(100%-16px)]  w-full md:rounded-2xl flex md:flex-col items-center justify-center md:justify-between md:py-8 gap-4 md:gap-0 md:px-0 p-3'>
            ...
            <div className='flex md:flex-col gap-4'>
                {MenuPages.map((page, index) =>
                    <div className={`w-12 h-12 rounded-xl outline-none ${activePage == index ? "bg-[#191A20] pointer-events-none text-white " : "text-[#191A20]"}`} key={index}>
                        <a className='h-full group/page w-full flex justify-center items-center' href={page.link}>
                            <page.icon className='text-lg ' size={32} />
                            <span className='group-hover/page:block z-[9999] absolute hidden left-[78px]  bg-white text-[#191A20] [box-shadow:rgba(0,_0,_0,_0.07)_0px_0.796192px_1.43315px_-0.75px,_rgba(0,_0,_0,_0.07)_0px_2.41451px_4.34611px_-1.5px,_rgba(0,_0,_0,_0.063)_0px_6.38265px_11.4888px_-2.25px,_rgba(0,_0,_0,_0.047)_0px_20px_36px_-3px]
